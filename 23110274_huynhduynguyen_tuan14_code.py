...
def draw_state(states, is_belief=False):
    """Draw one or two puzzle states with improved UI."""
    screen.fill(WHITE)
...
def get_state_key(state):
    """Chuyển trạng thái thành khóa có thể băm cho Q-table."""
...
def take_action(state, action):
    """Thực hiện hành động trên trạng thái và trả về trạng thái mới."""
    x_0, y_0 = Node(state).get_index_0()
    moves = {
        "up": (-1, 0),
        "down": (1, 0),
...
def reward_function(state):
    """Xác định phần thưởng cho trạng thái hiện tại."""
    if state == final_state:
        return 100
    return -1 - heuristic(state) / 10

def perturb_state(state, steps=1):
    """Biến đổi trạng thái bằng cách thực hiện một vài di chuyển ngẫu nhiên hợp lệ."""
...
def load_q_table():
    """Tải Q-table từ tệp nếu tồn tại."""
    global Q_table
...
def train_q_learning(initial_state, episodes=2000, max_steps=200):
    """Huấn luyện Q-table, tập trung vào trạng thái ban đầu và các trạng thái lân cận."""
    global Q_table
    start_time = time.perf_counter()
...
def q_learning(state, episodes=2000, max_steps=200):
    """Sử dụng Q-table đã huấn luyện để tìm đường đi giải."""
    start_time = time.perf_counter()
...
def draw_buttons(selected_algo):
    """Draw buttons with organized layout."""
    buttons = [
